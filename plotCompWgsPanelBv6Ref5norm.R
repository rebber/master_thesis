...
#  text(PTEN_genomestart,3,"PTEN",pos=4,offset=0.1,cex=1.2)
  ...
  plot(dat_cnr$startcoord, dat_cnr$log2, xaxt="n", pch=16, col='#00000090', cex=0.3, ylim=c((-3.5),3.5), main=title2,xlab="Chromosome #", ylab="log2 CN ratio",cex.main=1,cex.axis=1,cex.lab=1)
...
#  text(PTEN_genomestart,3,"PTEN",pos=4,offset=0.1,cex=1.2)
  ...
  #plot segmented values as bars
  dat_cns_pos <- dat_cns[which(dat_cns$log2>0),]
  dat_cns_neg <- dat_cns[which(dat_cns$log2<0),]
  dat_wgs_pos <- dat_wgs[which(log2(dat_wgs$segmented)>0),]
  dat_wgs_neg <- dat_wgs[which(log2(dat_wgs$segmented)<0),]
  
  #wgs
  plot(dat_wgs$startcoord, log2(dat_wgs$copynumber), xaxt="n", type="n", ylim=c((-3.5),3.5),xlab="Chromosome #", ylab="log2 CN ratio",cex.main=1,cex.axis=1,cex.lab=1)
  rect(dat_wgs_pos$startcoord,rep(0,length(dat_wgs_pos$startcoord)),dat_wgs_pos$endcoord,log2(dat_wgs_pos$segmented),col="red",lty=0)  
  rect(dat_wgs_neg$startcoord,rep(0,length(dat_wgs_neg$startcoord)),dat_wgs_neg$endcoord,log2(dat_wgs_neg$segmented),col="blue",lty=0)  
  abline(v=c(chr_coord$start,chr_coord$end[24]), h=0, lwd=0.5) #vertical lines for the chromosomes and horizontal for neutral log2CN  
  axis(1, at=xticks, label=c(1:22,"X","Y"),cex.axis=1)
  #panel
  plot(dat_cnr$startcoord, dat_cnr$log2, xaxt="n", type="n", ylim=c((-3.5),3.5),xlab="Chromosome #", ylab="log2 CN ratio",cex.main=1,cex.axis=1,cex.lab=1)
  rect(dat_cns_pos$startcoord,rep(0,length(dat_cns_pos$log2)),dat_cns_pos$endcoord, dat_cns_pos$log2,col="red",lty=0)
  rect(dat_cns_neg$startcoord,rep(0,length(dat_cns_neg$log2)),dat_cns_neg$endcoord, dat_cns_neg$log2,col="blue",lty=0)
  abline(v=c(chr_coord$start,chr_coord$end[24]), h=0, lwd=0.5) #vertical lines for the chromosomes and horizontal for neutral log2CN  
  axis(1, at=xticks, label=c(1:22,"X","Y"),cex.axis=1)
