...
files_cnr <- dir(pattern="cnr", paste(path_CNVkit,"batchResultsRef5",sep=""), recursive = TRUE)
...
for (i in 1:length(files_cnr)) {
  ...
  dat_cnr <- read.table(paste(path_CNVkit,"batchResultsRef5/", files_cnr[i], sep=""), header = TRUE, as.is=TRUE)
  ...
  write.table(dat_cnr,paste(path_CNVkit,"batchResultsRef5/cnrRcComb/",file_path_sans_ext(files_cnr[i]),".txt",sep=""),quote=FALSE,row.names=FALSE)
}