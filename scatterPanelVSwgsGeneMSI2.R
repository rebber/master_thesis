...
#Here for MSI2-samples
#Created by Rebecka Bergström on 2016-03-10, based on scatterPanelVSwgsGeneMSI.R

library(MASS)
library(tools)

path_CNVkit <- "/home/rebecka.bergstrom/cnvkit/MSI-PILOT2/100kbATbins/full_tumors/"
setwd(paste(path_CNVkit, "Rplots/scatter_panelVSwgs",sep=""))

files_wgs <- dir(pattern="wgs.genes.bed", paste(path_CNVkit,"segments2genes/",sep=""), recursive = TRUE)
files_panel <- dir(pattern="panel.genes.bed",paste(path_CNVkit,"segments2genes/",sep=""), recursive=TRUE)
...
for (i in 1:length(files_wgs)) {
  data_wgs <- read.table(paste(path_CNVkit,"segments2genes/",files_wgs[i],sep=""),as.is=TRUE)
  ...
  data_panel <- read.table(paste(path_CNVkit,"segments2genes/",files_panel[i],sep=""), as.is=TRUE)
  ...
  plot(log2CN_wgs_remextr,log2CN_panel_remextr, pch=16, col='#00000050',cex=0.5, main=title2, cex.main=0.8, xlab="log2 CN wgs", ylab="log2 CN panel",ylim=c(-3.5,3.5),xlim=c(-3.5,3.5))
...
pdf(filename,width=10,height=10)
plot(log2CN_wgs_remextr_all,log2CN_panel_remextr_all, pch=16, col='#00000050',cex=0.5, main=title2, cex.main=0.8, xlab="log2 CN wgs", ylab="log2 CN panel",xlim=c(-3.5,3.5),ylim=c(-3.5,3.5))
