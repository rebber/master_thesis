#Plot comparison of the segments and bins from CNVkit for diluted panel data and from QDNAseq for wgs data, for Bv7 samples with homozygos PTEN deletion
...
for (j in 2:10) {
  ...
  #------------------------------
  #Change to the prefered paths and tumor percentage here
  tumPerc <- tumPerc_steps[j] #the percentage of tumor reads in diluted bam files
  path_CNVkit <- paste("/home/rebecka.bergstrom/cnvkit/BREASTv7_PTEN_del/homozDelPTEN/panelV1_CORRECT/dilutions/tumorPerc_",tumPerc, "/", sep="") #where the CNVkit-files are to be found and plots to be saved
  path_wgs <- "/home/Crisp/clinseq/BREASTv7/" #where the wgs-files are to be found
  samples <- c("PTENdel1","PTENdel2","PTENdel3","PTENdel4") #exchange PTENdel1 etc towards actual sample IDs
  #------------------------------
  
  setwd(paste(path_CNVkit, "Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl", sep=""))
  ...
  for (i in 1:length(samples)) {
    files_wgs <- append(files_wgs,dir(pattern=paste(samples[i],"T_wgs.qdnaseq.txt",sep=""),path_wgs,recursive=TRUE))
  }
  ...
  for (i in 1:length(files_cnr)) {
    ...
    dat_cns <- read.table(paste(path_CNVkit, "batchResults/", files_cns[i], sep=""), header = TRUE, as.is=TRUE)
    ...
    title_wgs <- paste("BREASTv7 log2 copy number chr 10 ",basename(file_path_sans_ext(files_wgs[i])),sep="")
    title_wgs_zoomIn <- "zoom in on PTEN, wgs"
    title_panel <- paste("BREASTv7 log2 copy number chr 10 diluted ",tumPerc, "% ", basename(file_path_sans_ext(files_cnr[i])),".cnvkit",sep="")
    title_panel_zoomIn <- "zoom in on PTEN, panel"
    filename=paste("Bv7_PTENdel_compWgsPanel_diluted_",tumPerc,"_",basename(file_path_sans_ext(file_path_sans_ext(gsub("_wgs","",files_wgs[i])))),".pdf",sep="")
    ...
    plot(chr10_wgs$start, log2(chr10_wgs$copynumber), xaxt="n",pch=16, col='#00000090', cex=0.3, ylim=c((-3.5),3.5), main=title_wgs,xlab="Position on Chr 10",cex.main=1, ylab="log2 CN wgs")
    ...
    plot(chr10_cnr$start, chr10_cnr$log2, xaxt="n", pch=16, col='#00000099', cex=0.3, ylim=c((-3.5),3.5), main=title_panel,xlab="Position on Chr 10",cex.main=1, ylab="log2 CN panel")
    segments(chr10_cns$start, chr10_cns$log2, chr10_cns$end, chr10_cns$log2, lwd=4, col="#66CD00")
    abline(v=PTEN_genestart, h=0, lwd=0.5) #vertical line for start of PTEN and horizontal for neutral CN
    abline(v=c(chr10_wgs$start[startZoomIn],chr10_wgs$start[endZoomIn]),lty=3)
    axis(1, at=seq(0,1.4e+08,20000000))
  ...
  saveRDS(test1_panel_collection,paste(path_CNVkit,"Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl/test1_panel",tumPerc,".rds",sep=""))
  saveRDS(test2_panel_collection,paste(path_CNVkit,"Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl/test2_panel",tumPerc,".rds",sep=""))
  saveRDS(PTENbins_panel_collection,paste(path_CNVkit,"Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl/PTENbins_panel",tumPerc,".rds",sep=""))
  saveRDS(surrPTEN1_panel_collection,paste(path_CNVkit,"Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl/surrPTEN1_panel",tumPerc,".rds",sep=""))
  saveRDS(surrPTEN2_panel_collection,paste(path_CNVkit,"Rplots/segmentPlotsComp/wilcoxontest/5x2Mbctrl/surrPTEN2_panel",tumPerc,".rds",sep=""))
